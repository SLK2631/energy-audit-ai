...
function buildReport(d, completedActions) {
  ...
  <div class="sec"><div class="st">Recommendations</div><table><thead><tr><th>Status</th><th>Category</th><th>Action</th><th>Est. Savings</th><th>Difficulty</th></tr></thead><tbody>${recs.map(r=>{const aid=`${d.provider}::${r.cat}::${r.title}`;const done=completedActions&&[...completedActions].some(k=>k.includes(r.title));return`<tr class="${done?'done':''}"><td>${done?'✓ Done':'Pending'}</td><td>${r.cat}</td><td style="font-weight:600">${r.title}</td><td style="color:#059669;font-weight:700;font-family:monospace">${r.estimatedSavings}</td><td><span style="background:${r.difficulty==='Easy'?'#d1fae5':r.difficulty==='Medium'?'#fef3c7':'#fee2e2'};color:${r.difficulty==='Easy'?'#065f46':r.difficulty==='Medium'?'#92400e':'#991b1b'};padding:2px 8px;border-radius:4px;font-weight:600;font-size:10px">${r.difficulty}</span></td></tr>`;}).join("")}</tbody></table>
  <div class="sv2"><div><div class="sv2l">Monthly Savings Potential</div><div class="sv2v">${d.totalPotentialMonthlySavings}</div></div><div><div class="sv2l">Annual Savings Potential</div><div class="sv2v">${d.totalPotentialAnnualSavings}</div></div></div></div>
